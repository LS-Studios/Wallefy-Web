import {useSettings} from "../../Providers/SettingsProvider";
import {DatabaseRoutes} from "../../Helper/DatabaseRoutes";
import {useMemo} from "react";
import {useCurrentUser} from "./useCurrentUser";
import AccountsScreen from "../../UI/Screens/Accounts/AccountsScreen";
import {AccountVisibilityType} from "../../Data/EnumTypes/AccountVisibilityType";

export const useAccountsRoute = (accountVisibility?: AccountVisibilityType) => {
    const settings = useSettings()

    const getAccountsRoute = () => {
        if ((accountVisibility || settings?.currentAccountVisibility) === AccountVisibilityType.PRIVATE) {
            return `${DatabaseRoutes.USER_ACCOUNTS}/${settings!.currentUserUid}/${DatabaseRoutes.PRIVATE_ACCOUNTS}`
        } else {
            return `${DatabaseRoutes.PUBLIC_ACCOUNTS}`
        }
    }

    const value = useMemo(() => getAccountsRoute, [accountVisibility, settings])

    if (!settings || !settings.currentUserUid || !settings.currentAccountUid) return null

    return value
}