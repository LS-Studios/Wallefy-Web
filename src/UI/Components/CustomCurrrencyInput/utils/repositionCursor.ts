...
export const repositionCursor = ({
  ...
  if (stateValue && cursorPosition) {
    ...
    if (lastKeyStroke === 'Delete' && stateValue[cursorPosition] === groupSeparator && stateValue[cursorPosition] === groupSeparator) {
      splitValue.splice(cursorPosition, 1);
      cursorPosition += 1;
    }

    const decimalPosition = stateValue.indexOf(decimalSeparator!);

    if (lastKeyStroke !== 'Backspace' && lastKeyStroke !== 'Delete' && decimalPosition > -1 && cursorPosition > (decimalPosition + 1)) {
      cursorPosition += 1
    }
